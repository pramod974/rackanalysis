...
class rules:
    ...
    def computeMissingLiftedWeeklyAndDailyOneDayNRD(self,df2,grp,i):
        if grp.ix[i+1]["('lifted_gallons', 'Weekly')"] != 0 and grp.ix[i+1]["('lifted_gallons', 'Daily')"]!=0:
            if grp.ix[i-1]["('lifted_gallons', 'Weekly')"] != 0 and grp.ix[i-1]["('lifted_gallons', 'Daily')"]!=0:
                ...
                df2.loc[str(grp.ix[i]['date']),"('lifted_gallons', 'Weekly')"]=newweekly
    ...
    def verifyWeekByNRD(self,df2):
        try:
            ...
            # df2.insert(df2.keys().get_loc("('lifted_gallons', 'Daily')"),"lifted_gallons_daily_modified",df2["('lifted_gallons', 'Daily')"].values)
            # df2.insert(df2.keys().get_loc("lifted_gallons_daily_modified"),"lifted_gallons_daily_flag",0)
            # df2.insert(df2.keys().get_loc("('lifted_gallons', 'Weekly')"),"lifted_gallons_weekly_modified",df2["('lifted_gallons', 'Daily')"].values)
            # df2.insert(df2.keys().get_loc("('lifted_gallons', 'Weekly')"),"lifted_gallons_weekly_flag",0)
            ...
            for key,grp in grpNRD:
                ...
                if grp["('lifted_gallons', 'Daily')"].cumsum()[-1]==grp["('lifted_gallons', 'Weekly')"][-1]:
                    ...
                    validLiftedGallons=df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"lifted_gallons_daily_modified"].cumsum()==df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"('lifted_gallons', 'Weekly')"]
                        ...
                        if df2.loc[str(grp.ix[i]['date']),"lifted_gallons_daily_flag"]==0:
                            if i!=0:
                                if grp.ix[i-1]["('lifted_gallons', 'Weekly')"] != 0 and grp.ix[i]["('lifted_gallons', 'Weekly')"] != 0:
                                    ...
                                    if df2.loc[str(grp.ix[i]['date']),"('lifted_gallons', 'Daily')"] != (grp.ix[i]["('lifted_gallons', 'Weekly')"]-grp.ix[i-1]["('lifted_gallons', 'Weekly')"]):
                                        df2.loc[str(grp.ix[i]['date']),"lifted_gallons_daily_modified"]=grp.ix[i]["('lifted_gallons', 'Weekly')"]-grp.ix[i-1]["('lifted_gallons', 'Weekly')"]
                                ...
                                if key==firstNRD and  firstNRD!=0 :
                                    ...
                                    if nrdGroups[firstNRD]!=7:
                                        ...
                                        df2.loc[str(grp.ix[i]['date']),"lifted_gallons_daily_modified"]= grp.ix[i]["('lifted_gallons', 'Weekly')"]
                                        df2.loc[str(grp.ix[i]['date']),"lifted_gallons_daily_flag"]=3
                    ...
                    if key==firstNRD and nrdGroups[firstNRD]!=7:
                        currentMonthCumLGD=df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"lifted_gallons_daily_modified"].cumsum()[-1]
                        presentMonthLastWeeklyLifted=df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"('lifted_gallons', 'Weekly')"][-1]
                        if (openingBalances['lifted_weekly']+currentMonthCumLGD)==presentMonthLastWeeklyLifted:
                            print "Modification_Successful"
                        else:
                            print "Modification_UnSuccessful"
                        cumSum=df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"lifted_gallons_daily_modified"].cumsum()
                        cumSum=cumSum.apply(lambda x:x+openingBalances['lifted_weekly'])
                        validLiftedGallons=cumSum==df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"('lifted_gallons', 'Weekly')"]
                        df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"sanityWeekly_CumulativeDaily_NextRefreshDate"]=validLiftedGallons
                    else:
                        if df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"lifted_gallons_daily_modified"].cumsum()[-1]==df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"('lifted_gallons', 'Weekly')"][-1]:
                            print "Modification_Successful"
                        else:
                            print "Modification_UnSuccessful"
                        validLiftedGallons=df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"lifted_gallons_daily_modified"].cumsum()==df2.loc[df2["('next_refresh_date', 'Weekly')"]==key,"('lifted_gallons', 'Weekly')"]
    ...
    def computeMissingLiftedMonthlyAndDailyOneDayNRD(self,df2,ind):
            ...
            if df2.ix[nextDayIndex]["('lifted_gallons', 'Monthly')"] != 0 and df2.ix[nextDayIndex]["('base_gallons', 'Daily')"]!=0:
                if df2.ix[previousDayIndex]["('lifted_gallons', 'Monthly')"] != 0 and df2.ix[previousDayIndex]["('base_gallons', 'Daily')"]!=0:
                    ...
                    df2.loc[str(df2.ix[ind]['date']),"('lifted_gallons', 'Monthly')"]=newMonthly
    ...
    def executeRules(self):
        try:
            ...
            self.df2.insert(self.df2.keys().get_loc("('lifted_gallons', 'Daily')"),"lifted_gallons_daily_modified",self.df2["('lifted_gallons', 'Daily')"].values)
            self.df2.insert(self.df2.keys().get_loc("lifted_gallons_daily_modified"),"lifted_gallons_daily_flag",0)
            self.df2.insert(self.df2.keys().get_loc("('lifted_gallons', 'Weekly')"),"Lifted_actual_weekly",self.df2["('lifted_gallons', 'Weekly')"].values)
            self.df2.insert(self.df2.keys().get_loc("('lifted_gallons', 'Weekly')"),"lifted_gallons_weekly_flag",0)
            self.df2.insert(self.df2.keys().get_loc("('lifted_gallons', 'Monthly')"),"Lifted_actual_monthly",self.df2["('lifted_gallons', 'Monthly')"].values)
            self.df2.insert(self.df2.keys().get_loc("('lifted_gallons', 'Monthly')"),"lifted_gallons_monthly_flag",0)
            ...
            if "('lifted_gallons', 'Monthly')" in self.df2.keys():
                ...
                self.df2.loc[self.df2.loc[:,"lifted_gallons_daily_modified"].cumsum()==self.df2.loc[:,"('lifted_gallons', 'Monthly')"],"sanityMonthly_CumulativeDaily_WeeksByNextRefreshDate"]=1
...
class ruleEngine:
    ...
    def createPivot(self,frame):
        ...
        if len(lgm)>0:
            frms.append(lgm)
        lgm=frame.loc[(frame["Modified_WeeksByLiftedGallons"]==1)|(frame["lifted_gallons_daily_flag"]==1),["date","account_type","supplier_terminal_name","product_name","('lifted_gallons', 'Daily')"]]
...
if __name__ == "__main__":
    ...
    for i in suppliers:
        detailedList=["pilot",i,'01-07-2015',r"C:\Users\pramod.kumar\Documents\rackanalysis_proj\rackanalysis\AnalysisRules"]
        ...
        # executeEngine.runRules((('PILOT TRAVEL CENTERS LLC 103637 IW', 'GLENDIVE MT CENEX EX - 02PT', 'ULSD'),))
